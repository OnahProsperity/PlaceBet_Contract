...
contract BSCGame {
    ...
    uint256 public bnbPrice = 0.1E18;
    uint256 public bcgPrice = 5E18;
    ...
    function giftBSG(uint256 _amount) public {
        if(tokensSold == 1000000E18) {
            bnbPrice = 0.6E18;
        }
        // require(saleActive == true, "WE ARE CURRENTLY OUT OF SALES");
        require(_amount > 0, "MUST BE GREATER THAN ZERO");
        uint256 cost = _amount * 10E18;
        uint256 tokensToGet = _amount * bcgPrice / bnbPrice * 10**18;
        // IERC20(busd).approve(address(this), cost);
        // require(IERC20(busd).transferFrom(_msgSender(), address(this), cost), "BSCG: TRANSACTION FAIL");
        require(IERC20(bscg).transfer(_msgSender(), tokensToGet), "CONTRACT DOES NOT HAVE ENOUGH TOKENS.");
        
        tokensSold += tokensToGet;
        emit Sale(_msgSender(), cost, tokensToGet);
    }
